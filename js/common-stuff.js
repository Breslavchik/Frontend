...
function createTestimonialFromUsers(users){
    const actualUsers=users.slice(5,6);
    const usersData = actualUsers.map(user=> {
        return {
...
// const testimonialMarkUp=usersData.reduce((res,data)=>res+createUsersTestimonial(data),'');
const testimonialContainer=document.querySelector('background');
// testimonialContainer.innerHTML=testimonialMarkUp;
usersData.forEach(item=>testimonialContainer.innerHTML+=createUsersTestimonial(item));
}
